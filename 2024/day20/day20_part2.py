...
for start_idx, cheat_start in enumerate(path):
    for end_idx, cheat_end in enumerate(path):
        dist = abs(cheat_start[0] - cheat_end[0]) + abs(cheat_start[1] - cheat_end[1])
        if dist <= 20 and (cheat_start, cheat_end) not in skips and (cheat_end, cheat_start) not in skips and abs(start_idx - end_idx) - dist >= 100:
